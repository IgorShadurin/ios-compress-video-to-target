...
enum VideoCompressionServiceError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .exportSessionUnavailable:
            L10n.tr("System export session is unavailable for this video.")
        case .exportFailed(let message):
            L10n.fmt("System export failed: %@", message)
        }
    }
}
...
final class VideoCompressionService {
    ...
    func exportLowQualityFallback(sourceURL: URL) async throws -> URL {
        ...
        guard let exportSession = AVAssetExportSession(
            ...
            throw VideoCompressionServiceError.exportSessionUnavailable
        }

        let preferredFileTypes: [AVFileType] = [.mp4, .mov, .m4v]
        let outputFileType = preferredFileTypes.first(where: { exportSession.supportedFileTypes.contains($0) })
            ?? exportSession.supportedFileTypes.first
        guard let outputFileType else {
            throw VideoCompressionServiceError.exportSessionUnavailable
        }

        let outputURL = makeOutputURL(container: CompressionContainer(fileType: outputFileType))
        ...
        try await withCheckedThrowingContinuation { continuation in
            exportSession.exportAsynchronously {
                ...
                case .failed:
                    continuation.resume(
                        throwing: VideoCompressionServiceError.exportFailed(
                ...
                default:
                    continuation.resume(
                        throwing: VideoCompressionServiceError.exportFailed(
                            exportSession.error?.localizedDescription ?? "Export did not complete"
                        )
                    )
                }
            }
    ...
    func compress(
        ...
        let writerBaseDimensions = VideoGeometry.writerBaseDimensions(
        ...
        let outputSize = calculateOutputSize(
            width: writerBaseDimensions.width,
            height: writerBaseDimensions.height,
